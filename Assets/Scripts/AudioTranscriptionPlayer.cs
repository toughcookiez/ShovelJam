...
public class AudioTranscriptionPlayer : MonoBehaviour
{
    ...
    AudioClip CreateClipWithSilenceIntro(AudioClip originalClip, float silenceDuration)
    {
        ...
        Debug.Log($"Created new clip: {newClip.name} with silence: {silenceDuration}s. Total length: {newClip.length}s");
    ...
    void Update()
    {
        ...
        if (_audioSource.time >= currentEntry.timestamp)
        {
            ...
            Instantiate(_notePrefab, transform.position + (Vector3.left * currentEntry.lane * 2), Quaternion.identity, _noteHolder.transform);
        }
